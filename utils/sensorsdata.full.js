...
var custom_mpshow_time=null; //自定义埋点app启动时间
...
_.getSessionId =function(data){
   var result;
   result=wx.getStorageSync('JSESSIONID')||wx.getStorageSync('sessionid')||wx.getStorageSync('SESSIONID')||
   wx.getStorageSync('token')||wx.getStorageSync('TOKEN')||"";
   if(!wx.getStorageSync('wx_sdk_session_id')||wx.getStorageSync('wx_sdk_session_id')!=result){
     wx.setStorageSync('wx_sdk_session_id', result);
     wx.setStorageSync('cov_begin', new Date().getTime());
     wx.setStorageSync('launch', 0);
     wx.setStorageSync('event', 0);
...
_.formatMytrackData= function(data){
  //  var obj={};
  //   obj
}
...
_.info = {
  ...
  getSystem: function () {
    ...
    function getNetwork() {
      wx.getNetworkType({
        "success": function (t) {
          // console.log("ttt-------getSystemInfo-------ttt:", t)

          // e.$network_type = t["networkType"];
          e.conn = t["networkType"] === "wifi" ? t["networkType"] : "cellular";
          e.cellular = t["networkType"] === "wifi" ? "" : t["networkType"];
        },
    ...
    function getSystemInfo() {
      wx.getSystemInfo({
        "success": function (t) {
          // console.log("ttt-------getSystemInfo-------ttt:", t);
          e.brand = t["brand"];  //品牌
          e.model = t["model"];     //手机硬件型号   
          e.manufacturer=t["brand"];  //厂商
          e.product="";
          // e.$screen_width = Number(t["screenWidth"]);
          // e.$screen_height = Number(t["screenHeight"]);
          e.screen = Number(t["screenWidth"]) + "*" + Number(t["screenHeight"])
          e.os = formatSystem(t["platform"]);
          e.os_version= t["system"]; //系统版本
          e.os_sdk = t["SDKVersion"]; //小程序基础库版本
        },
        "complete": function () {
...
sa.prepareData = function (p, callback) {
  ...
  var data = {
    distinct_id: this.store.getUnionId().anonymous_id,
    rzh:this.store.getDistinctId(),
    // lib: {
      $lib: LIB_NAME,
      $lib_method: 'code',
      $lib_version: String(LIB_VERSION),
  ...
  if (!p.type || p.type.slice(0, 7) !== 'profile') {
      data.track_id = Number(String(Math.random()).slice(2, 5)  + String(Date.now()).slice(-4)) +String(Math.random()).slice(2, 4);
    ...
    data.timezone="UTC+"+(0-new Date().getTimezoneOffset()/60);
     //去获取经纬度
    data.gis_lat="";
    data.gis_lng="";
    wx.getSetting({
      success(res) {
        if (res.authSetting['scope.userLocation']) {
         wx.getLocation({
             success(res){
                 data.gis_lat=res.latitude;
                 data.gis_lng=res.longitude;
             }
         })
        }
      }
    })
    data.previous = sa_referrer;
    data.device_id = sa.store._state.device_id;
  ...
  _.extend(data, data.properties);
  delete data.properties;
  // data=_.formatMytrackData(data);
...
sa.store = {
  ...
  getDeviceId:function(){
    return "" + Date.now() + '-' + Math.floor(1e7 * Math.random()) + '-' + Math.random().toString(16).replace('.', '') + '-' + String(Math.random() * 45231).replace('.', '').slice(0, 8);
  },
  getStorage: function () {
    if (this.storageInfo) {
...
sa.track = function (e, p, c) {
  // _.getSessionId();
  var oldEvent=wx.getStorageSync('event');
  wx.setStorageSync('event', oldEvent+1);
  //定义可以被覆盖的字段
   var prop={
    //  previous:sa_referrer
    // wx.setStorageSync('cov_begin', new Date().getTime());
    
    cov_begin:wx.getStorageSync('cov_begin'),
    cov_end:cov_end||new Date().getTime(),
    timebegin:timebegin||"",
    timeend:timeend||"",
    switch_count:switch_count||0,
    launch:wx.getStorageSync('launch')||0,
    event:wx.getStorageSync('event')||0,
...
sa.init=function (config) {
  
  sa.setPara({
    server_url: 'https://wx-test.zhangle.com/collect/web',
    name: "myTrack",
    show_log: config.show_log ? true : false,
    autoTrack: {
      appLaunch: config.autoTrack ? true : false,
      appShow: config.autoTrack ? true : false,
      appHide: config.autoTrack ? true : false,
      pageShow: config.autoTrack ? true : false,
      pageShare: config.autoTrack ? true : false,
      // 默认不采集，设置为 true 可采集预置点击事件
      mpClick: config.autoTrack ? true : false
    },
  });
 
  sa.registerApp({
    channel_env: config.channel_env ? config.channel_env : "dev",
    browser:"wechatdevtools",  //浏览器
    ...
    event:"",
    page:"",
    product_type:config.product_type?config.product_type:"",    //系统类别
    product_id:config.product_id?config.product_id:"",      //系统id
    product_name:config.product_name?config.product_name:"",   
    recvtime:"",
    ip:"",
    ip_country:"",
    ip_province:"",
...
sa.event_report=function (p) {
  
  p = _.isObject(p) ? p : {};
  var params = _.deepClone(p);
  delete params.sysType;
  delete params.id;
  delete params.target;
   if(p.id=="system"){
      if(p.sysType=="start"){
        sa.autoTrackCustom.c_appLaunch(wx.getLaunchOptionsSync(),params);
      }
      if(p.sysType == "in"){
        sa.autoTrackCustom.c_appShow(wx.getLaunchOptionsSync(),params);
      }
      if(p.sysType=="out"){
        sa.autoTrackCustom.c_appHide(params);
      }
   }else if(p.id=="page"){
      // var propPage={};
      // propPage.page_id=_.getCurrentPath();
      sa.autoTrackCustom.c_pageShow(params);
      
   } else if(p.id=="webclick"){
      var prop={
        page_id:_.getCurrentPath(),
        previous:sa_referrer
      };
      if(p.target==undefined||p.target==null||p.target==""){
        sa.track("webclick",_.extend(prop,params));
      }
      else{
        var target = p.target.currentTarget || {};
        var dataset = target.dataset || {};
        // var type =p.target['type'];
        prop['btn_id'] = target.id?target.id:"";
        prop['btn_title'] = dataset['name'];
        sa.track("webclick",_.extend(prop,params));
      }
    
   }else {
     sa.track("other",_.extend({
      page_id:_.getCurrentPath(),
      previous:sa_referrer
     },params))
   }
...
sa.user_set=function(set_type,set_params,set_profile){
  if(set_type ==="login"){
    if(set_params) {
      sa.login(set_params)
    ...
    if(set_profile){
      sa.registerApp(set_profile)
    }
  }
  if(set_type ==="logout"){
    cov_end=new Date().getTime();
    sa.logout();
  }
}
...
sa.sendStrategy = {
  ...
  queueSend: function (url) {
    ...
    if (sa.para.server_url.indexOf('?') !== -1) {
      ...
      url = sa.para.server_url + '&data=' + url;
    } else {
      // url = sa.para.server_url + '?data=' + encodeURIComponent(_.base64Encode(url));
      // url = sa.para.server_url + '?data=' + url;
      _.wxrequest({
...
sa.autoTrackCustom = {
  ...
  appLaunch: function (para, not_use_auto_track) {
      console.log( "applaunch"+sa_referrer);
      timebegin=new Date().toISOString();
      _timebegin=new Date().getTime();
     var oldlaunch=wx.getStorageSync('launch');
     wx.setStorageSync('launch', oldlaunch+1);
    //  wx.setStorageSync('timebegin', new Date().getTime())
    ...
    if (para && para.path) {
      ...
      prop.page_id= _.getPath(para.path);
    }
    prop.previous=sa_referrer;
  ...
  appHide: function (not_use_auto_track) {
    ...
    // }
    prop.use_time=current_time-_timebegin;
    switch_count= switch_count+1;
  ...
  pageShow: function () {
    console.log( "pageshow"+sa_referrer);
    var oldpage=wx.getStorageSync('page');
    wx.setStorageSync('page', oldpage+1);
    var prop = {};
    var router = _.getCurrentPath();
  ...
  c_appLaunch:function (para, not_use_auto_track) {
    ...
    wx.setStorageSync('launch', oldlaunch+1);
     ...
     sa.track("system", prop);
    
   },
  c_appShow: function (para, not_use_auto_track) {
    var prop = {};
  ...
  c_pageShow: function (not_use_auto_track) {

    var oldpage=wx.getStorageSync('page');
    wx.setStorageSync('page', oldpage+1);

    not_use_auto_track= _.isObject(not_use_auto_track)?not_use_auto_track:{};
    var prop = {};
    var router = _.getCurrentPath();
    prop.$referrer = sa_referrer;
    prop.previous=sa_referrer;
  ...
  c_appHide: function (not_use_auto_track) {
    var current_time = (new Date()).getTime();
    var prop = {};
    prop.$url_path = _.getCurrentPath();
    prop.page_id= _.getCurrentPath();
